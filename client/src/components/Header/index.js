...
export default function Header() {
  ...
  return <header className="flex items-center gap-3 border-b border-slate-200 bg-white px-5 py-3 dark:border-slate-800 dark:bg-slate-900"><form onSubmit={submit} className="flex w-full max-w-xl items-center gap-3 rounded-lg bg-slate-100 px-3 py-2 dark:bg-slate-800"><Search size={18} className="text-slate-400" /><input value={query} onChange={(event) => setQuery(event.target.value)} className="w-full bg-transparent text-sm text-slate-900 outline-none placeholder:text-slate-400 dark:text-slate-100" placeholder="Search mail" aria-label="Search mail" /></form><ThemeToggle /></header>;
}
